...
function Header() {
  ...
  return (
    ...
    <header className='bg-background h-20 flex justify-between items-center p-2 w-full'>
      <div>
        <Link to="/map">
          <img src='/images/logo_letter2.png' alt='로고' className='w-20 md:w-24 sm:w-20 bg-background rounded-none p-1' />
        </Link>
      </div>
      <div className="flex items-center relative">
        <Link to="/mypage/myPage">
          <div className="w-16 md:w-24 sm:w-20 p-3">
            <img src={profileImage} alt='프로필' className='w-full h-full rounded-full border border-gray-300' />
          </div>
        </Link>
        <div className='flex flex-col'>
          <button onClick={toggleMenu} className="font-extrabold text-primary py-2 px-1 sm:py-2 sm:px-1 md:py-2 md:px-2 text-xs sm:text-sm md:text-baserounded-full relative">
            ▽
            {isMenuOpen && (
